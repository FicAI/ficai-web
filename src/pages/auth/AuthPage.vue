  ...
  <q-page class="row items-center justify-evenly">
    <q-card
      ...
      <q-card-section class="">
        <div class="text-h3 text-bold text-center">fic.ai</div>
      </q-card-section>
      <q-separator />
      <q-card-section>
        <q-btn-group spread unelevated>
          <q-btn
            :to="{ path: 'login', query: { next: $route.query.next } }"
            :outline="routePathLast() !== 'login'"
            color="primary"
            label="Log in" />
          <q-btn
            :to="{ path: 'register', query: { next: $route.query.next } }"
            :outline="routePathLast() !== 'register'"
            color="primary"
            label="Register" />
        </q-btn-group>
        <q-btn
          ...
          color="primary"
          label="OR"
          class="absolute-center dark-button">
